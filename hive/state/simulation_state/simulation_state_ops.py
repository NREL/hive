...
def add_request(sim: SimulationState,
                request: Request) -> Tuple[Optional[Exception], Optional[SimulationState]]:
    """
    ...
    if not sim.road_network.geoid_within_geofence(request.origin):
        return SimulationStateError(
            f"origin {request.origin} not within road network geofence"), None
    else:
...
def remove_request(sim: SimulationState,
    ...
    if not request:
        ...
        updated_r_locations = DictOps.remove_from_collection_dict(sim.r_locations, request.geoid,
                                                                  request.id)
        updated_r_search = DictOps.remove_from_collection_dict(sim.r_search, search_geoid,
                                                               request.id)
...
def modify_request(
    ...
    if not request:
        ...
        result = DictOps.update_entity_dictionaries(updated_request, sim.requests, sim.r_locations,
                                                    sim.r_search, sim.sim_h3_search_resolution)
...
def add_vehicle(sim: SimulationState,
                vehicle: Vehicle) -> Tuple[Optional[Exception], Optional[SimulationState]]:
    """
    ...
    if not sim.road_network.geoid_within_geofence(vehicle.geoid):
        ...
        updated_v_locations = DictOps.add_to_collection_dict(sim.v_locations, vehicle.geoid,
                                                             vehicle.id)
...
def add_vehicle_returns(sim: SimulationState,
                        vehicle: Vehicle) -> Result[SimulationState, Exception]:
    """
    adds a vehicle into the region supported by the RoadNetwork in this SimulationState
...
def modify_vehicle(
    ...
    if not vehicle:
        ...
        updated_dictionaries = DictOps.update_entity_dictionaries(updated_vehicle, sim.vehicles,
                                                                  sim.v_locations, sim.v_search,
                                                                  sim.sim_h3_search_resolution)

        updated_sim = sim._replace(
            vehicles=updated_dictionaries.entities
            if updated_dictionaries.entities else sim.vehicles,
            v_locations=updated_dictionaries.locations
            if updated_dictionaries.locations else sim.v_locations,
            v_search=updated_dictionaries.search if updated_dictionaries.search else sim.v_search)
        return None, updated_sim


def remove_vehicle(sim: SimulationState,
    ...
    if not isinstance(vehicle_id, VehicleId):
        error = SimulationStateError(
            f"remove_request() takes a VehicleId (str), not a {type(vehicle_id)}")
        return error, None
        ...
        updated_sim = sim._replace(
            ...
            v_locations=DictOps.remove_from_collection_dict(sim.v_locations, vehicle.geoid,
                                                            vehicle_id),
...
def add_station(sim: SimulationState,
                station: Station) -> Tuple[Optional[Exception], Optional[SimulationState]]:
    """
    ...
    if not sim.road_network.geoid_within_geofence(station.geoid):
        ...
        updated_s_locations = DictOps.add_to_collection_dict(sim.s_locations, station.geoid,
                                                             station.id)
...
def remove_station(sim: SimulationState,
                   station_id: StationId) -> Tuple[Optional[Exception], Optional[SimulationState]]:
    """
    ...
    if not station:
        ...
        search_geoid = h3.h3_to_parent(station.geoid, sim.sim_h3_search_resolution)
        updated_s_locations = DictOps.remove_from_collection_dict(sim.s_locations, station.geoid,
                                                                  station_id)
        updated_s_search = DictOps.remove_from_collection_dict(sim.s_search, search_geoid,
                                                               station_id)
...
def add_base(sim: SimulationState,
             base: Base) -> Tuple[Optional[Exception], Optional[SimulationState]]:
    """
...
def remove_base(sim: SimulationState,
                base_id: BaseId) -> Tuple[Optional[Exception], Optional[SimulationState]]:
    """
    ...
    if not base:
        ...
        search_geoid = h3.h3_to_parent(base.geoid, sim.sim_h3_search_resolution)
        updated_b_locations = DictOps.remove_from_collection_dict(sim.b_locations, base.geoid,
                                                                  base_id)
...
def modify_base(sim: SimulationState,
                updated_base: Base) -> Tuple[Optional[Exception], Optional[SimulationState]]:
    """
