...
from hive.util.typealiases import RequestId, VehicleId, BaseId, StationId, SimTime
...
def add_request(sim: SimulationState, request: Request) -> Tuple[Optional[Exception], Optional[SimulationState]]:
    ...
    if not sim.road_network.geoid_within_geofence(request.origin):
        ...
        updated_sim = sim._replace(
            ...
            request_iterator=sim.request_iterator + (request,),
            r_locations=DictOps.add_to_location_dict(sim.r_locations, request.geoid, request.id),
            r_search=DictOps.add_to_location_dict(sim.r_search, search_geoid, request.id)
        )
...
def remove_request(sim: SimulationState,
    ...
    if not request:
        ...
        updated_r_locations = DictOps.remove_from_location_dict(sim.r_locations, request.geoid, request.id)
        updated_r_search = DictOps.remove_from_location_dict(sim.r_search, search_geoid, request.id)

        updated_sim = sim._replace(
            requests=updated_requests,
            request_iterator=tuple(r for r in sim.request_iterator if r.id != request_id),
...
def add_vehicle(sim: SimulationState, vehicle: Vehicle) -> Tuple[Optional[Exception], Optional[SimulationState]]:
    ...
    if not sim.road_network.geoid_within_geofence(vehicle.geoid):
        ...
        updated_sim = sim._replace(
            ...
            vehicle_iterator=sim.vehicle_iterator + (vehicle,),
...
def remove_vehicle(sim: SimulationState, vehicle_id: VehicleId) -> Tuple[
    ...
    if not isinstance(vehicle_id, VehicleId):
        ...
        updated_sim = sim._replace(
            ...
            vehicle_iterator=tuple(v for v in sim.vehicle_iterator if v != vehicle_id),
            v_locations=DictOps.remove_from_location_dict(sim.v_locations, vehicle.geoid, vehicle_id),
            v_search=DictOps.remove_from_location_dict(sim.v_search, search_geoid, vehicle_id)
        )
        return None, updated_sim
...
def add_station(sim: SimulationState, station: Station) -> Tuple[Optional[Exception], Optional[SimulationState]]:
    ...
    if not sim.road_network.geoid_within_geofence(station.geoid):
        ...
        updated_sim = sim._replace(
            ...
            station_iterator=sim.station_iterator + (station,),
...
def remove_station(sim: SimulationState, station_id: StationId) -> Tuple[
    ...
    if not station:
        ...
        updated_sim = sim._replace(
            ...
            station_iterator=tuple(s for s in sim.station_iterator if s.id != station_id),
...
def add_base(sim: SimulationState, base: Base) -> Tuple[Optional[Exception], Optional[SimulationState]]:
    ...
    if not sim.road_network.geoid_within_geofence(base.geoid):
        ...
        updated_sim = sim._replace(
            ...
            base_iterator=sim.base_iterator + (base,),
...
def remove_base(sim: SimulationState, base_id: BaseId) -> Tuple[Optional[Exception], Optional[SimulationState]]:
    ...
    if not base:
        ...
        updated_sim = sim._replace(
            ...
            base_iterator=tuple(b for b in sim.base_iterator if b.id != base_id),
