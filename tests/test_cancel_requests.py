...
class TestCancelRequests(TestCase):
    def test_update_cancellable(self):
        req = mock_request()
        sim_or_err = simulation_state_ops.add_request_safe(
            mock_sim(sim_time=600), req
        )
        ...
        self.assertNotIn(
            req.id, result.requests, "request should have been removed"
        )
        self.assertNotIn(
            req.origin,
            result.r_locations,
    ...
    def test_update_not_cancellable(self):
        ...
        result, _ = cancel_requests.update(sim, env)
        self.assertIn(
            req.id, result.requests, "request should not have been removed"
        )
        self.assertIn(
            req.origin,
            result.r_locations,
