...
def mock_station(
        ...
        h3_res: int = 15,
        chargers=None,
        on_shift_chargers=None,
        road_network: RoadNetwork = mock_network(),
        membership: Membership = Membership(),
) -> Station:
    if chargers is None:
        chargers = immutables.Map({mock_l2_charger_id(): 1, mock_dcfc_charger_id(): 1})
    if on_shift_chargers is None:
        on_shift_chargers = frozenset(chargers.keys())
    return Station.build(station_id, h3.geo_to_h3(lat, lon, h3_res), road_network, chargers, on_shift_chargers, membership)
...
def mock_station_from_geoid(
    ...
    if on_shift_chargers is None:
        on_shift_chargers = frozenset(chargers.keys())
    return Station.build(station_id, geoid, road_network, chargers, on_shift_chargers, membership)
