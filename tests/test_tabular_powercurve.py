...
class TestTabularPowercurve(TestCase):
    ...
    def test_leaf_energy_gain_0_soc(self):
        ...
        energy_source = EnergySource("test_id",
                                     ...
                                     capacity=50,  # kilowatthour
                                     energy=0,  # kilowatthour
                                     ideal_energy_limit=40,  # kilowatthour
                                     max_charge_acceptance_kw=50,  # kilowatt
                                     )

        result = leaf_model.refuel(energy_source, Charger.DCFC, 2 * HOURS_TO_SECONDS)
    ...
    def test_leaf_energy_gain_full_soc(self):
        ...
        energy_source = EnergySource("test_id",
                                     energy_type=EnergyType.ELECTRIC,
                                     capacity=50,  # kilowatthour
                                     energy=50,  # kilowatthour
                                     ideal_energy_limit=50,  # kilowatthour
                                     max_charge_acceptance_kw=50,  # kilowatt
                                     )
        self.assertTrue(energy_source.is_at_ideal_energy_limit(), "test precondition should be at max")
        result = leaf_model.refuel(energy_source, Charger.DCFC, 1 * HOURS_TO_SECONDS)
    ...
    def test_leaf_energy_gain_low_power(self):
        ...
        result = leaf_model.refuel(energy_source, Charger.LEVEL_2, 1 * HOURS_TO_SECONDS)
