...
class TestIntializeOps(TestCase):
    def test_process_fleet_file(self):
        fleets_file_location = resource_filename(
        ...
        self.assertEqual(
            len(veh_member_ids["v13"]), 1, "v3 should be a member of only one fleet",
        )

        self.assertEqual(
            len(base_member_ids["b1"]), 1, "b1 should only be a member of one fleet",
        )

        self.assertEqual(
            len(station_member_ids["s1"]), 1, "s1 should be only be a member of one fleet",
        )
