...
class TestDictReaderStepper(TestCase):
    ...
    def test_reading_up_to_a_time_bounds_should_leave_remaining_file_available(self,):
        ...
        for row in result:
            self.assertLess(
                SimTime.build(row["departure_time"]), stop_time, f"should be less than {stop_time}",
            )
        stepper.close()
    ...
    def test_reading_two_consecutive_times(self):
        ...
        self.assertEqual(
            len(result1), 20, f"should have found 20 rows with departure time [0, {stop1})",
        )
        self.assertEqual(
            len(result2), 5, f"should have found 5 rows with departure time [{stop1}, {stop2})",
        )
        for row in result1:
            self.assertLess(
                SimTime.build(row["departure_time"]), stop1, f"should be less than {stop1}",
            )
        for row in result2:
            self.assertGreaterEqual(
                SimTime.build(row["departure_time"]), stop1, f"should be gte {stop1}",
            )
            self.assertLess(
                SimTime.build(row["departure_time"]), stop2, f"should be less than {stop2}",
            )
        stepper.close()
    ...
    def test_no_second_file_reading_on_repeated_value(self):
        ...
        self.assertEqual(
            len(result1), 20, f"should have found 20 rows with departure time earlier than {stop}",
        )
        self.assertEqual(
            len(result2), 0, f"will not advance file by calling same stop value {stop}",
        )
    ...
    def test_correct_management_of_stored_value_after_repeated_value(self):
        ...
        self.assertEqual(
            len(result), 1, f"should have found 20 rows with departure time earlier than {stop2}",
        )
        self.assertEqual(
            SimTime.build(stepper._iterator.history["departure_time"]),
            stop2,
