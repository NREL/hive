...
def calc_veh_stats(veh_fleet):
    veh_ids, trip_vmts, dispatch_vmts, total_vmts, reqs_filled = [], [], [], [], []
    pass_del, pct_time_trips, refuel_cnts, refuel_secs, idle_secs, end_soc = [], [], [], [], [], []
    for veh in veh_fleet:
        veh_ids.append(veh.veh_id)
        trip_vmts.append(veh.trip_vmt)
        dispatch_vmts.append(veh.dispatch_vmt)
        total_vmts.append(veh.total_vmt)
        reqs_filled.append(veh.requests_filled)
        pass_del.append(veh.passengers_delivered)
        pct_time_trip = veh.trip_s / (veh.refuel_s + veh.idle_s + veh.dispatch_s + veh.trip_s)
        pct_time_trips.append(pct_time_trip)
        refuel_cnts.append(veh.refuel_cnt)
        refuel_secs.append(veh.refuel_s)
        idle_secs.append(veh.idle_s)
        end_soc.append(veh.soc)
    
    veh_stats_df = pd.DataFrame(data={'veh_id': veh_ids,
                                      'trip_vmt': trip_vmts,
                                      'dispatch_vmt': dispatch_vmts,
                                      'total_vmt': total_vmts,
                                      'requests_filled': reqs_filled,
                                      'passengers_delivered': pass_del,
                                      'pct_time_trip': pct_time_trips,
                                      'refuel_count': refuel_cnts,
                                      'refuel_seconds': refuel_secs,
