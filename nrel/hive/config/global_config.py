...
from nrel.hive.reporting.reporter import ReportType
...
class GlobalConfig(NamedTuple):
    ...
    def from_dict(cls, d: Dict, global_settings_file_path) -> GlobalConfig:
        ...
        output_base_directory_absolute = (
            Path(d["output_base_directory"]).expanduser()
            if d["output_base_directory"].startswith("~")
            else Path(d["output_base_directory"])
        )
        d["output_base_directory"] = str(output_base_directory_absolute)

        # convert list of logged report types to a Set
        d["log_sim_config"] = (
            set(ReportType.from_string(rt) for rt in d["log_sim_config"])
            if d["log_sim_config"]
            else set()
        )
    ...
    def write_outputs(self):
        return (
            ...
            or self.log_events
            or self.log_stats
            or self.log_station_capacities
            or self.log_instructions
            or self.log_time_step_stats
            or self.log_fleet_time_step_stats
        )
